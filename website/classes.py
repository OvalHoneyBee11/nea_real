...
def my_classes():
    ...
    class_memberships = ClassMembership.query.filter_by(user_id=current_user.id).all()
    enrolled_classes = [m.class_obj for m in class_memberships]
...
def create_class():
    ...
    if request.method == "POST":
        ...
        existing_class = Class.query.filter_by(
            name=name, teacher_id=current_user.id
        ).first()
        if existing_class:
            flash(
                f"You already have a class named '{name}'. Please choose a different name.",
                "warning",
            )
            return render_template(
                "create_class.html",
        ...
        new_class = Class(
            name=name, description=description, teacher_id=current_user.id, code=code
        )
...
def join_class():
    ...
    if request.method == "POST":
        ...
        if not class_code:
            ...
            if not class_obj:
                flash(
                    "Invalid class code. Please check and try again.", category="error"
                )
            elif class_obj.teacher_id == current_user.id:
                flash(
                    "You are the teacher of this class! You're already a member.",
                    category="error",
                )
            else:
                ...
                if existing:
                    ...
                    flash(
                        f"Successfully joined '{class_obj.name}'!", category="success"
                    )
                    return redirect(
                        url_for("classes.class_detail", class_id=class_obj.id)
                    )
...
def class_detail(class_id):
    ...
    students = (
        User.query.join(ClassMembership)
...
def class_chat(class_id):
    ...
    if request.method == "POST":
        ...
        if not message_text or not message_text.strip():
            ...
            new_message = ChatMessage(
                message=message_text.strip(), user_id=current_user.id, class_id=class_id
            )
    ...
    messages = (
        ChatMessage.query.filter_by(class_id=class_id)
